...
void CollisionSystem::CheckCollision(level &level, float dt)
{
	...
	for (Entity& entity : entities)
	{
		...
		if (entity.getName() == "Flag")
		{
			...
			pickupcomp = (PickUpComponent *)entity.getCompByType("PickUp");
	
		}
		
		if (posComp1 != NULL && posComp2 != NULL) {
			std::cout << pickupcomp->getState() << std::endl;
			if (AABB(posComp1->getPositionX(), posComp1->getPositionY(), posComp2->getPositionX(), posComp2->getPositionY(),
				...
				if (cc->hasFlag == false && pickupcomp->getState() == pickupcomp->Collectable) {
					pickupcomp->setState(pickupcomp->NotCollectable);
					cc->hasFlag = true;


				}	
				if (cc->hasFlag == true && pickupcomp->getState() == pickupcomp->NotCollectable)
				{
